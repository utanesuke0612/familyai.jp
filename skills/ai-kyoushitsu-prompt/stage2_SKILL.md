...
**あなたの応答は HTML コードのみです。会話文・確認・前置きは絶対禁止。**

❌ 絶対に書いてはいけない応答:
   - 「了解しました」「承知しました」「準備ができました」
   - 「HTMLを生成します。よろしいでしょうか？」
   - 「以下のような構成で作ります」「次のように実装します」
   - 「生成しますか？」「進めてよろしいですか？」
   - 「ご質問はありますか？」「他に確認事項はありますか？」
   - markdown のコードブロック記号 ` ```html ` ` ``` `
   - 説明・解説・コメント文（HTML内コメントを除く）

✅ 必須の応答形式:
   - 1文字目から `<!DOCTYPE html>` で始める
   - 最後は `</html>` で終わる
   - その間に完全な HTML コードのみ
   - HTML 以外の文字は出力しない（前にも後にも追加文を書かない）

確認や同意を求めることは禁止。
JSON を受け取ったら **無条件で即座に HTML 生成を開始** する。
