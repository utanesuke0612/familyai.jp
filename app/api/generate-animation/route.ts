...
async function callStage1Api(
   ...
   * 既存 system prompt は単発リクエスト前提のため、会話履歴・options 必須化・
   * 英文禁止のルールを system message の最上位に挿入する。
   */
...
- 「生成してよろしいですか？」「Would you like me to generate?」
  「よろしいでしょうか？ HTMLを生成します」のような事前確認は完全禁止
- 確認せず黙って success を返すのが正しい挙動

【C. 会話履歴の解釈（最重要）】
このリクエストは、システムメッセージの後に「会話履歴」が assistant / user
ロールの交互メッセージとして含まれている場合がある。これは継続中の会話である。
最新の user メッセージは単独のテーマではなく、それまでの assistant 発言の
続きとして解釈すること。

特に、最新の user メッセージが以下のような短い了承・続行表現の場合：
...
→ 絶対に新規テーマとして扱わず、直前の assistant 発言で確認していたテーマを採用する
→ そのテーマで success JSON を返す（needs_clarification にしない・error にもしない）

例：
  assistant: 「天気の変化 - 雲の動きと雨の関係」のテーマで作成します。よろしいでしょうか？
  user: これでOK
  → 「天気の変化 - 雲の動きと雨の関係」のテーマで success JSON を返す（必須）
...
`;
  ...
  // Phase 1c+: 会話履歴を **マルチターン message 配列** として LLM に渡す。
  // 文字列で埋め込むより、LLM 標準の chat 形式の方が「会話」と認識されやすく、
  // history 解釈の精度が上がる。
  ...
  try {
    return await completeOpenRouter(
      ...
      // PRIORITY_RULES を厳格に守らせる。
      { maxTokens: 4000, temperature: 0, signal: controller.signal },
    );
  } finally {
    clearTimeout(timer);
  }
