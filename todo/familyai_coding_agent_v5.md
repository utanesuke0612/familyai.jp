...
> 最終更新: 2026-04-17（Step 01〜12 完了・Rev01〜06 完了）
...
| Step 13: `/api/audio` をVercel Blob署名URL方式で実装・再生カウント重複防止（`/api/audio/play`）を追加                                                         | CodingAgent | 2026-05-08まで | 40分  | Todo01, Todo09, Todo18                                                 | 未着手 |
...
| **Todo02**: OpenRouter でAPIキーを発行し `OPENROUTER_API_KEY` を設定（**AI呼び出しの最優先経路。OpenRouterで不足する場合のみ個別キーを追加取得**）                             | 人間（junli）   | 2026-05-08まで | —    | —                                                                      | 未着手 |
...
| **Todo18**: Vercel Blob を作成し `BLOB_READ_WRITE_TOKEN` を設定（Vercel Marketplace → Storage → Blob → `familyai-blob`）                       | 人間（junli）   | 2026-05-08まで | —    | —                                                                      | 未着手 |
