...
> 最終更新: 2026-04-19 22:30（Rev23 完了＋Rev23.1/23.2 Vercel hotfix 完了・Vercel 本番デプロイ成功・Phase QA-T1 スモークテスト 44/45 PASS）
...
| **Phase QA-T2（Vitest ユニットテスト）: `shared/utils`（buildQueryString 配列対応）／zod スキーマ（createArticleSchema・updateArticleSchema）／repositories の ILIKE エスケープ純粋関数を対象。見積 2〜3時間。** | CodingAgent | Phase 1 並行 | 約150分 | — | ⏳未着手 |
| **Phase QA-T3（API 統合テスト）: 管理者セッションをモックして admin CRUD 正常系＋rate limit 11req/min → 429 を検証。`/api/ai` の free/pro プラン別挙動も対象。テスト用 DB（Neon branch）を利用。見積 3〜4時間。** | CodingAgent | Rev24 着手前 | 約210分 | — | ⏳未着手 |
...
| **Phase QA-CI（GitHub Actions 連携）: `.github/workflows/test.yml` で push/PR ごとに tsc + lint + Vitest + Playwright を自動実行。Vercel preview URL に対する E2E 連携も視野。見積 1時間。** | CodingAgent | QA-T2 完了後 | 60分 | — | ⏳未着手 |
| **Rev24 🟡（Phase 4 / iOS・Android 着手前・合計約4時間）: モバイル API 拡充＋高トラフィック最適化の第2波。①`GET /api/articles/latest` と `GET /api/articles/[slug]/related` を新規実装（`getLatestArticles` / `getRelatedArticles` をラップ・モバイル記事トップ/詳細の等価 API／45分）。②`POST /api/auth/mobile/token` を新規実装（Sign in with Apple + Google ID Token 検証→JWT 発行・NextAuth Cookie に依存しないモバイル認証導線／90分）。③`viewCount` を Upstash Redis `INCR` に切替＋Vercel Cron で毎分 DB へバッチ flush（hot row contention 解消／60分）。④`listAllArticles` に pagination（limit/offset）追加＋`AdminArticleTable` 側を無限スクロール or ページ送りに（記事 1000 件超え対応／40分）。⑤`verifyCsrf` に `X-Client-Platform` ヘッダー＋モバイル API キーの明示許可リストを追加（20分）。** | CodingAgent | Phase 4 着手時 | 約240分 | — | ⏳Phase 4 待機 |
...
> **CodingAgent 合計推定時間: 約 1360分（22.7時間）** ※Rev07〜23.2＋QA-T1 を含む（QA-T2〜CI・Rev24/25 は別枠）
> **完了済み**: Step 01〜22 全て・Rev01〜23.2 全て・Phase QA-T1 スモーク 🎉
> **未着手（Phase QA）**: QA-T2（Vitest）／QA-T3（API統合）／QA-T4（Playwright E2E）／QA-CI（GitHub Actions）
