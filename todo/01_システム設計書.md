...
|---|---|---|---|---|
| **Neon PostgreSQL** | RDB本体（記事・ユーザー・AI教室生成履歴・AIメモ・単語ブックマーク・AI設定）— 詳細は §4 | `DATABASE_URL` | `lib/db/index.ts` 経由で全API | Phase1 |
| **Vercel Blob** | CDN付きオブジェクト保存（音声 MP3 / OGP 画像キャッシュ / 将来アバター） | `BLOB_READ_WRITE_TOKEN` | `app/api/audio/route.ts`（読取）<br>直接アップロードは管理者のみ | Phase1 |
| **Git リポジトリ（ファイル）** | 静的コンテンツ（VOA レッスン Markdown / AI 教室プロンプトテンプレート）— DB 不使用 | — | `lib/voaenglish/lessons.ts`<br>`app/api/generate-animation/route.ts` | Phase1 |
| **ブラウザ localStorage** | 非ログインユーザーの一時データ（AI メモ・単語ブックマーク） | — | `lib/ai-memo-store.ts`<br>`lib/voaenglish/vocab-store.ts` | Phase1 |
...
│
└── todo/                         📝 仕様・作業管理
    ├── 01_システム設計書.md            このドキュメント
    ├── 02_CodingAgent指示書.md         CodingAgent 実装依頼書
    ├── 03_手作業手順書.md              人間作業手順書
    ├── 04_テスト仕様書.md              テスト観点14カテゴリ・80+項目（Phase QA）
    └── テスト結果/                     テスト実行ログ・レビュー記録
        ├── TestResult_202604192224.md    Phase QA-T1 スモーク結果（44/45 PASS）
        ├── TestResult_202604192243.md    Phase QA-T2/CI 結果（Vitest 43/43 PASS）
        ├── TestResult_20260420_1529.md   Rev26 実装後の検証結果
        ├── Codexレビュー.md                Codex による初回レビュー指摘
        ├── Codexレビュー_v2(修正後のレビュー).md  Rev26 対応後の再レビュー
        ├── Codexレビュー結果に対してClaudeの見解.md  Rev26 着手前の検証記録
        └── Rev27_実装指示.md              Rev27 着手時の実装指示書
```
...
familyai.jp は **5 つのストレージ層**にデータを分散配置している。各層の役割を明確に分離することで、コスト・スケーラビリティ・プライバシーのバランスを取っている。

### 4-1. ストレージ層の早見表

| # | ストレージ | 永続性 | 主な用途 | 月額目安（現状規模） |
|---|---|---|---|---|
| 1 | **Neon PostgreSQL** | 永続 | 記事・ユーザー・学習データ（7 テーブル） | $0（無料枠 0.5GB） |
| 2 | **Vercel Blob** | 永続 | 音声 MP3・OGP 画像キャッシュ | $0〜$5 |
| 3 | **Upstash Redis** | 揮発（TTL） | レート制限カウンタのみ | $0（無料枠） |
| 4 | **Git リポジトリ** | 永続（バージョン管理） | VOA レッスン .md・AI教室プロンプト | $0 |
| 5 | **ブラウザ localStorage** | クライアント側 | 非会員のメモ・単語ブックマーク | クライアント負担 |

> 🎯 **設計原則**: 揮発データは Redis、構造化永続データは Neon、ファイル系は Blob、コンテンツは Git、非会員データは localStorage。**役割が混ざらないよう徹底**。

---

### 4-2. Neon PostgreSQL — 7 テーブル構成

#### 🔧 システム / 運用データ（管理者・記事編集者が操作）

| テーブル | 主なカラム | 操作経路 | 件数イメージ |
|---|---|---|---|
| **`articles`** | `id, slug, title, body (Markdown), categories[], level, thumbnail_url, view_count, is_featured, published, published_at` | `/admin/articles` で管理者が CRUD | 数十〜数百 |
| **`ai_config`** | `id (=1 固定), config (jsonb・Partial<AiKyoshitsuConfig>), updated_at, updated_by` | `/admin/ai-config` で管理者が編集 | 1 行限定 |
| **`ai_config_history`** | `id, config (jsonb), changed_at, changed_by, change_note` | PUT 時に自動 INSERT・直近 10 件保持 | 最大 10 件 |

#### 👥 ユーザーアカウント

| テーブル | 主なカラム | 認証経路 |
|---|---|---|
| **`users`** | `id, email, name, image, auth_provider ('google'\|'local'\|'apple' 将来), password_hash (bcrypt 12), plan ('free'\|'premium'), stripe_customer_id` | NextAuth v5（Google OAuth or Credentials） |

#### 📚 ユーザー学習データ（ログイン会員のクラウド同期）

すべて `userId FK ON DELETE CASCADE` — **アカウント削除と同時に消える設計**。

| テーブル | 主なカラム | UI 経路 | 備考 |
|---|---|---|---|
| **`user_animations`** | `id, user_id, theme, grade, subject, prompt, html_content, is_favorite (R3-U1), custom_title (R3-U1), is_public (R3-K3), created_at` | `/mypage/ai-kyoshitsu` | AI 教室生成 HTML を **DB 全文保存**（Blob 不使用） |
| **`user_ai_memos`** | `id, user_id, memo_id, question, answer, article_title, article_slug, saved_at` | `/mypage/aimemo` | unique(user_id, memo_id) で重複排除 |
| **`user_vocab_bookmarks`** | `id, user_id, vocab_id, word, meaning, pron, example, course, lesson, lesson_title, added_at` | `/mypage/vocab` | unique(user_id, vocab_id) で重複排除 |
...
┌──────────────────────────┐         ┌──────────────────────────┐
│ articles                 │         │ users                    │
│  記事本体（管理者管理）   │         │  アカウント（認証情報）   │
└──────────────────────────┘         └─────────┬────────────────┘
                                               │ 1:N (CASCADE)
                                  ┌────────────┼────────────┐
                                  ▼            ▼            ▼
                          ┌─────────────┐┌──────────┐┌──────────────┐
                          │user_animat. ││user_ai_  ││user_vocab_   │
                          │AI教室履歴   ││memos     ││bookmarks     │
                          │  + ⭐/🔒     ││AIメモ    ││単語ブックマーク│
                          └─────────────┘└──────────┘└──────────────┘

┌──────────────────────────┐         ┌──────────────────────────┐
│ ai_config (id=1 固定)    │ N:1     │ ai_config_history        │
│  AI教室パイプライン設定   │ 履歴    │  直近 10 件・PUT 時記録   │
└──────────────────────────┘         └──────────────────────────┘
```

> 🛡️ **プライバシー**: 認証情報（password_hash）は bcrypt ハッシュ保存。生 IP・PII は DB に保存しない。  
> 📊 **マイグレーション履歴**: `drizzle/000X_*.sql` で管理。最新は `0012_user_animations_is_public.sql`（R3-K3）。

---

### 4-3. Vercel Blob — ファイル配信層

| パス prefix | 内容 | 使用箇所 | 配信方法 |
|---|---|---|---|
| `audio/` | TTS で生成した記事読み上げ音声 MP3 | `/api/tts` で生成・`/api/audio` で配信 | 署名付き URL（短期）|
| `og/` | OGP 画像のキャッシュ（`/api/og` 動的生成結果） | 記事一覧・詳細ページの og:image | CDN キャッシュ |
| `avatar/` | （予約・将来用） | ユーザーアイコンアップロード予定 | — |

> ⚠️ **注意**: AI 教室の生成 HTML は Blob ではなく **Neon DB 直接保存**（`user_animations.html_content`）。1 アニメ ≈ 30〜50KB なので 1 万件で 500MB。将来的に Blob 移行検討。

---

### 4-4. Upstash Redis — 揮発カウンタ

レート制限用のキーバリューストア。**TTL でデータが自動消滅**するため永続データは保管しない。

| キー prefix | 上限 | 用途 |
|---|---|---|
| `ratelimit:anim:free` | 3 / 日 / userId | AI 教室生成（無料会員） |
| `ratelimit:anim:pro` | 100 / 日 / userId | AI 教室生成（有料会員） |
| `ratelimit:chat:free` | 30 / 日 | AI チャット（無料） |
| `ratelimit:chat:pro` | 200 / 日 | AI チャット（有料） |
| `ratelimit:tts` | 5 / 分 / IP | TTS API |
| `ratelimit:og` | 30 / 時 | OGP 画像生成 |
| `ratelimit:register` | 5 / 10分 / IP | 新規登録（ボット対策） |
| `ratelimit:admin` | 10 / 分 / userId+IP | 管理 API |

> Redis が落ちても**機能は動く**（fail-open ポリシー）。レート制限は best-effort。

---

### 4-5. Git リポジトリ（ファイルベース）

DB 化する程ではない静的コンテンツ・テンプレートは Git で管理し、ビルド時に同梱する。

| パス | 内容 | アクセス方法 |
|---|---|---|
| `content/voaenglish/*.md` | VOA レッスン本体（slug, course, level, audio URL, 本文）| `lib/voaenglish/lessons.ts` で `fs.readFileSync` + `gray-matter` |
| `skills/ai-kyoushitsu-prompt/*.md` | AI 教室パイプラインのプロンプトテンプレート（Stage1 / Stage2 / Legacy）| `app/api/generate-animation/route.ts` で読み込み・Vercel `outputFileTracingIncludes` で bundle 同梱 |

> 🔮 **AIctation 移行時の論点**: VOA レッスン .md → DB 化（`lessons` + `lessons_progress` テーブル）が必要。詳細は `todo/aictation_page_design.md` 参照。

---

### 4-6. ブラウザ localStorage（非会員データ）

非ログインユーザーでも基本機能を使えるようにするため、クライアント側にも保存層を持つ。

| キー | 内容 | ログイン時の同期 |
|---|---|---|
| `familyai:aimemo` | 非ログインユーザーの AI メモ | ログイン後に `user_ai_memos` へ移行 |
| `familyai:vocab` | 非ログインユーザーの単語ブックマーク | ログイン後に `user_vocab_bookmarks` へ移行 |

> 📜 **方針**: 非会員でも localStorage で動作 → ログインで自動的にクラウド同期。GDPR 等の負担を最小化。
...
【サーバ永続データ（main 層）】
┌────── Neon PostgreSQL (7 tables) ──────┐  ┌─ Vercel Blob ─┐
│                                         │  │                │
│ システム:                                │  │ audio/  ← MP3  │
│ ├ articles      (記事)                  │  │ og/     ← OGP  │
│ ├ ai_config     (AI設定 1行)            │  │ avatar/(予約)  │
│ └ ai_config_history (履歴 10件)         │  └────────────────┘
│                                         │
│ ユーザーアカウント:                      │  ┌── Upstash Redis ─┐
│ └ users         (会員情報)              │  │                   │
│                                         │  │ ratelimit:* (TTL) │
│ 学習保存（CASCADE 削除）:                │  └───────────────────┘
│ ├ user_animations    (AI教室履歴)       │
│ ├ user_ai_memos      (AIメモ)           │  ┌── Git Repo ────┐
│ └ user_vocab_bookmarks (単語)           │  │                  │
└─────────────────────────────────────────┘  │ content/         │
                                              │   voaenglish/.md │
                                              │ skills/          │
                                              │   ai-kyoushitsu/ │
...
┌── ブラウザ localStorage ──┐
│                            │
│ familyai:aimemo (非会員)   │ → ログイン時に DB へ移行
│ familyai:vocab  (非会員)   │
└────────────────────────────┘
...
| | 月額 | 主な要因 |
|---|---|---|
| Neon | **$0**（無料枠）| 0.5GB 以下 / 1 万行未満 |
| Vercel Blob | **$0〜$5** | TTS 音声蓄積量次第 |
| Upstash Redis | **$0**（無料枠）| 10,000 commands/day 以下 |
| Vercel Hosting | **$0〜$20** | 帯域・関数実行時間 |
| OpenRouter API | **$5〜$30** | AI 教室・チャットの利用量次第 |
| **合計** | **$5〜$55/月** | 現状規模 |

> 📈 **スケールポイント**: ユーザー数が 1 万人を超えると Neon Pro（$19/月）が必要。AI 教室の生成 HTML が 1 万件超えたら Blob 移行を検討。
...
- 実装依頼書: [todo/02_CodingAgent指示書.md](./02_CodingAgent指示書.md)
- 人間作業手順: [todo/03_手作業手順書.md](./03_手作業手順書.md)
- テスト計画: [todo/04_テスト仕様書.md](./04_テスト仕様書.md)
- テスト結果（T1 スモーク）: [todo/テスト結果/TestResult_202604192224.md](TestResult_202604192224.md)
- テスト結果（T2 Vitest + CI）: [todo/テスト結果/TestResult_202604192243.md](TestResult_202604192243.md)
