...
### 6c. AI教室パイプラインのチューニング（任意・運用調整用）
**所要時間: 必要に応じて 1〜5分**
**状態: 任意 — 未設定でも `AI_KYOSHITSU_DEFAULTS` の値で動作**

#### 概要

うごくAI教室の Stage1（テーマ詳細化）/ Stage2（HTML生成）のモデル・タイムアウト・
出力サイズ等を、Vercel Dashboard から **Deploy 不要・即時反映** で調整できる。

実装の流れ:
- 呼び出し側 (`app/api/generate-animation/route.ts`) は `await getAiConfig()` のみ
- 設定の優先順位: **DEFAULTS（コード）< env（Vercel Dashboard）**
- env を未設定にしておけば、コード内の `AI_KYOSHITSU_DEFAULTS` の値で安全に動く

#### 設定可能な環境変数（すべて任意）

| 変数名 | デフォルト値 | 何を変えたい時に設定 |
|---|---|---|
| `AI_STAGE1_MODEL` | `google/gemini-2.0-flash-001` | Stage1 モデルを変える時（例: 別の高速JSONモデルへ） |
| `AI_STAGE2_MODEL` | `google/gemini-2.0-flash-001` | Stage2 モデルを変える時（例: Haiku/Sonnet へ品質UP） |
| `AI_STAGE1_TIMEOUT_MS` | `8000` | Stage1 タイムアウト調整（単位: ms） |
| `AI_STAGE2_TIMEOUT_MS` | `50000` | Stage2 タイムアウト調整（Vercel 60秒制限内で） |
| `AI_STAGE2_MAX_TOKENS` | `6000` | 生成HTML サイズ上限の調整 |
| `AI_STAGE2_TEMPERATURE` | `0.5` | 出力のランダム性（0〜1）。低いほど確定的 |
| `CHAT_DEFAULT_MODEL` | `qwen/qwen3-14b` | AIチャット既定モデル |

**ソースコード参照**: `shared/constants/index.ts` の `AI_KYOSHITSU_DEFAULTS`、
`lib/config/ai-config.ts` の `getAiConfig()`

#### 設定手順

1. https://vercel.com/dashboard にアクセス
2. familyai-jp プロジェクト選択 → **Settings** タブ
3. 左メニュー → **Environment Variables**
4. **Add New** をクリック
5. 入力:
   - **Name**: 例 `AI_STAGE2_MODEL`
   - **Value**: 例 `anthropic/claude-3.5-haiku`
   - **Environment**: ☑ Production / ☑ Preview / ☑ Development（3つ全部チェック推奨）
6. **Save** をクリック
7. Vercel が自動再デプロイ（30秒〜1分で本番反映）

#### 設定変更後の挙動

```
Vercel Dashboard で env を Save
   ↓ （即座に）
Vercel が自動的に再デプロイ
   ↓ (30秒〜1分)
新しい env 値で本番が動く
...
**Git push もコード変更も一切不要**

#### 利用シナリオ例

| シナリオ | 設定例 |
|---|---|
| 品質を上げたい | `AI_STAGE2_MODEL=anthropic/claude-sonnet-4`<br>`AI_STAGE2_MAX_TOKENS=10000`<br>`AI_STAGE2_TIMEOUT_MS=55000` |
| コストを下げたい | `AI_STAGE2_MODEL=google/gemini-2.0-flash-001`（既定値）<br>`AI_STAGE2_MAX_TOKENS=4000` |
| タイムアウトが多い | `AI_STAGE2_TIMEOUT_MS=40000`<br>`AI_STAGE2_MAX_TOKENS=5000` |
| ロールバックしたい | 該当 env を **Delete** して保存<br>→ コード内の DEFAULTS 値に自動的に戻る |

#### 注意点

- ⚠️ **値の検証は無い**: env で設定した値はそのまま使われる
  （`AI_STAGE2_MAX_TOKENS=99999` も拒否されない）。
  極端な値は Vercel 60秒制限を超えてエラーになる可能性あり。
- ⚠️ **数値は文字列で保存される**: env はすべて文字列。
  `numOrUndef()` で数値変換し、失敗時は DEFAULTS にフォールバック。
- ⚠️ **シークレット類とは別**: `AI_STAGE2_MODEL` 等は公開可能な情報。
  ただし `OPENROUTER_API_KEY` などのシークレットと **同じ環境変数画面**
  で管理されるので、間違って公開設定にしないこと。
- ✅ **未設定が一番安全**: 必要になるまで設定しないことを推奨。
  デフォルト値（Gemini 2.0 Flash + Vercel 60秒制限内）は十分実用的。

#### 将来の拡張（Phase 2）

実装内部は **抽象化レイヤー** になっており、将来は以下が可能:
- `/admin/ai-config` 管理画面から DB 経由で編集（コード変更不要）
- Vercel Edge Config / Redis でリアルタイム反映（コード変更不要）
- A/B テスト（ユーザー%で振り分け・コード変更不要）

詳細は `lib/config/ai-config.ts` のコメントを参照。
