...
右: ナビリンク（PC）
  - 記事を読む（/learn） / ツール（/tools） / About（/about）
  - ※「共通ガイド」は廃止。/learn?cat=basic として記事カテゴリに統合済み。
  - ホバー: 下線アニメーション（width 0→100%）
  - CTAボタン「✨ 無料で始める」: orange背景・pill形状・shadow付き

モバイル: ボトムナビゲーション（4タブ固定）
  - 🏠 ホーム / 📖 記事 / 🛠️ ツール / 👤 マイページ
  - ハンバーガーメニューは使わない（スマホ最優先のため）
  - shadcn/ui Sheet はPCのサイドドロワー用途にのみ使用すること
```
...
記事詳細ページ・AIctationページの右サイドバーに配置するウィジェット。
**ページの種類によって2つのモードを切り替える。**
コンポーネントは1つ・modeプロップで制御する。

```typescript
// 使い分け:

// ① 普通の記事ページ（/learn/[slug]）→ シンプル版
<AIChatWidget
  mode="simple"
  context={article.body}
/>

// ② AIctationページ（/tools/aictation/[lessonId]）→ カテゴリタブ版
<AIChatWidget
  mode="aictation"
...
```
対象: /learn/[slug]（AI使い方ガイド記事）
カテゴリタブ: なし

外観:
  白背景・rounded-2xl・shadow-xl
  ヘッダー: 「🤖 AIに質問する」 orange accent
  入力欄: textarea（2行）+ 送信ボタン（orange）
  レスポンス表示エリア（スクロール可能）
  ローディング: 3点ドットアニメーション
...
API呼び出し:
  ...
  context: article.body（記事本文）
```

---

#### モード② カテゴリタブ版（AIctationページ用）

```
対象: /tools/aictation/[lessonId]（英語聴写ページ）
カテゴリタブ: 5つ
...
API呼び出し:
  POST /api/ai
  type: 'text-quality'（Claude Haiku・語学特化）
  context: lesson.script（VOAスクリプト全文）
...
② 記事カテゴリ:
   - はじめての方（/learn?cat=basic）← 旧「共通ガイド」はここに統合
   - 語学・英語（/learn?cat=language）
   - 家事・料理（/learn?cat=cooking）
   - 勉強・子育て（/learn?cat=study）
③ ツール:
   - 外語学習（/tools/language）
   - 九宮格ライティング（/tools/writing）
   - AI作図・音声（/tools → Pro機能）
④ サイト情報: Aboutページ / プライバシーポリシー / 利用規約
...
│   │   ├── page.tsx                  # トップページ
│   │   ├── learn/
│   │   │   ├── page.tsx              # 記事一覧（?cat=basic で入門記事に絞り込み可）
│   │   │   └── [slug]/page.tsx       # 記事詳細
│   │   ├── tools/
│   │   │   ├── page.tsx              # ツール一覧（無料・Pro区分表示）
│   │   │   └── [toolId]/page.tsx     # 各ツール画面
│   │   ├── apps/page.tsx             # 自作アプリ（Phase2）
│   │   └── about/page.tsx            # About
│   │   # ⚠️ /common は廃止。入門コンテンツは /learn?cat=basic に統合済み。
│   ├── api/
...
/learn?cat=basic                # 入門・はじめての方
                                # ⚠️ /common は廃止。旧「共通ガイド」はこのURLに統合。
/learn?level=beginner           # 初心者向け
/learn/[slug]                   # 記事詳細

/tools                          # ツール一覧
/tools/language                 # 外語学習ツール
/tools/writing                  # 九宮格ライティング
/tools/image                    # AI作図（Pro限定）
/tools/voice                    # 音声制作（Pro限定）
```
...
Step 14: app/(site)/tools/page.tsx（ツール一覧・無料/Pro区分表示）+ about/page.tsx + privacy/page.tsx + terms/page.tsx
# ⚠️ common/page.tsx は不要。入門記事は /learn?cat=basic に統合。
...
Step 23: PWA実装（manifest.json・Service Worker・オフラインページ）
         ← junliさんがアイコン画像を用意してからCodingAgentに依頼すること
```
...
<meta name="apple-mobile-web-app-title" content="familyai" />
...
スマホでネイティブアプリと同等の体験を提供するため、PWAを実装すること。
...
{
  "name": "familyai.jp — AI（愛）で、家族をもっと笑顔に。",
  "short_name": "familyai",
  "description": "パパ・ママ・子ども・シニア みんなのためのAI活用メディア",
  "start_url": "/",
  "display": "standalone",
...
作成方法:
  Canvaで familyai.jpのロゴを
  正方形にデザインして書き出す

  デザイン指針:
    背景色: #FF8C42（オレンジ）
    アイコン: 🏠 または familyai のロゴ
    余白: アイコンの周囲に20%の余白
    （maskableアイコンの要件）
```

#### ③ Service Worker（音声キャッシュと統合）

```javascript
// public/sw.js
// 音声キャッシュ（AIctation用）に加えて
// ページのオフライン対応も追加する

const CACHE_AUDIO = 'aictation-audio-v1';
const CACHE_PAGES = 'familyai-pages-v1';
...
--- AIChatWidget チェック ---
[ ] 記事ページ（/learn/[slug]）でシンプル版が表示される
[ ] シンプル版に定型質問ボタンが4つ表示される
[ ] シンプル版にカテゴリタブが表示されないこと
[ ] AIctationページでカテゴリタブ版が表示される
[ ] カテゴリタブ（📖📚📝✍️🎯）が5つ表示される
[ ] タブをクリックすると定型質問ボタンが切り替わる
[ ] 定型質問ボタンをタップすると入力欄に自動入力される
[ ] メッセージを送信するとAIが返答する
[ ] AIctation版はlesson.scriptがコンテキストに含まれる
[ ] ローディング中に3点ドットアニメーションが表示される
[ ] モバイルでカテゴリタブが横スクロールできる
...
[ ] /terms（利用規約）が表示される
[ ] /learn?cat=basic で入門記事が表示される（旧/commonの代替）
[ ] /common にアクセスすると /learn?cat=basic にリダイレクトされる
[ ] /tools のツール一覧が表示される
[ ] 無料ツールが使える（外語学習・九宮格ライティング）
[ ] Proツールがグレーアウト・「近日公開」表示になっている
[ ] スマホでボトムナビゲーション（4タブ）が表示される
...
--- PWAチェック ---
[ ] public/manifest.json が配置されている
[ ] public/icons/icon-192.png が配置されている（junliさん作成）
[ ] public/icons/icon-512.png が配置されている（junliさん作成）
[ ] iOSのSafariで「ホーム画面に追加」ができる
[ ] ホーム画面のアイコンをタップするとアプリ風に全画面で起動する
[ ] 起動時にスプラッシュ画面（オレンジ背景）が表示される
[ ] アドレスバーが非表示になる（standalone表示）
[ ] オフライン時に /offline ページが表示される
[ ] キャッシュ済みのMP3がオフラインでも再生できる
[ ] Chrome（Android）でも「ホーム画面に追加」が促される
