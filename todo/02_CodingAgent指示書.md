...
> 最終更新: 2026-05-07（**Rev32 完了 + Rev31 Phase 1 + 3 + 4 完了**: Rev32 で DeepSeek 公式 / Qwen DashScope 直 API 対応を実装。`lib/ai/provider-registry.ts` + `lib/ai/providers/openai-compatible.ts` 新設、`AiModelOption` に provider/nativeId 追加、新規 5 モデル登録、管理画面 provider バッジ + optgroup 化。`tsc 0 / lint 0 / test 17files 159 PASS`。Rev31: 同セッションで Phase 1（commit `754396a`）に加え Phase 3（CX-2/4/5/7/8/9）+ Phase 4（CX-3/11/12 + Codex Admin a11y）も実装し commit `c7dc44a` で push 待ち。`tsc --noEmit` 0 件・`pnpm lint` 0 件・`pnpm test` 146/146 PASS。新設ファイル: `lib/log.ts`（構造化ロガー）`lib/env.ts`（zod env 検証）`app/api/health/route.ts`（死活監視）`components/ui/ConfirmDialog.tsx`（a11y モーダル）。Phase 2（Next.js 15 + NextAuth beta.31）はローカル `pnpm add` がネットワーク不安定でハングしたため未完了・junli 環境での再実行を要請。Phase 5 未着手）<br>2026-05-05（**Rev31 起票・Phase 1 着手**: Codex レビュー19件 + Option A レビューを統合し Rev31 として段階対応プラン文書化。Phase 1（lock file 統一・packageManager pin・CI pnpm build job 追加）コミット済み。Phase 2-5 は Next.js 14→15 セキュリティアップグレード・安定性インフラ・a11y の順で別 PR 順次着手予定）<br>2026-04-20（Rev24 #① + #④ + #⑤ 実装完了・QA-T3 admin CRUD 統合テスト 14件追加で Vitest 79/79 PASS・`tsc --noEmit EXIT=0`・`pnpm lint` 0件／#① `GET /api/articles/latest` + `/related` 新規・`fetchLatest`/`fetchRelated` 追加／#④ admin pagination（listAllArticles に `{page,pageSize}` 拡張・`/api/admin/articles` GET で `data.items/meta` 返却・AdminArticleTable に前後ページ UI 追加）／#⑤ `verifyCsrf` を `{allowMobile:true}` 受付に拡張・`X-Client-Platform` + `X-Mobile-Api-Key`（env `MOBILE_API_KEYS` 許可リスト）検証を追加／QA-T3: `test/integration/admin-articles.integration.test.ts` で admin GET/POST/PUT/DELETE/PATCH-toggle の正常系＋ 403/400/404/429/CSRF 違反を vi.mock 戦略で検証／Rev27 実装完了：Codex v2 再レビュー 4項目すべて実装／Rev26 実装完了：Codex レビュー 11項目すべて実装／Rev23・23.1・23.2 完了＋Vercel 本番デプロイ成功・Phase QA-T1 スモーク 44/45 PASS＋QA-CI ワークフロー配置完了）
...
| **Rev25 🟢（記事10,000件規模になった時点・10,000+規模の全文検索高速化＋関連記事ランダム化）: `ilike '%search%'` の Sequential Scan 問題を段階的に解決。①pg_trgm 拡張 + GIN index（Neon で `CREATE EXTENSION pg_trgm`／記事1万件規模で有効）。②PostgreSQL Full Text Search (`to_tsvector` + 専用インデックス)。③必要に応じて Meilisearch / Typesense / Algolia を外付け。<br>④ **Rev29-C7 合流**：`lib/repositories/articles.ts:111` の `getRelatedArticles` の `ORDER BY random()` をフルスキャン回避型に変更。対応案A（推奨・1万件未満規模で十分だがこの Rev は1万件超を想定）: 関連候補を最大100件まで取得→アプリ層で `Math.random()` シャッフル→`limit` 件取り。対応案B（10万件超）: `count() * Math.random()` で offset を計算してから `LIMIT k OFFSET random_offset`。同時に `app/(site)/learn/[slug]/page.tsx` で `getRelatedArticles` を `cache()` でラップしてリクエスト内重複呼び出しを解消。** | CodingAgent | 記事10,000件到達時 | 約120分〜1日＋25分（C7） | — | ⏳規模到達待機 |
...
| **Rev28 🟡（アクセシビリティ強化・シニア配慮・Phase 2 着手時／約120分／ドラフト — junli 確認前）: 本文の文字サイズをユーザー自身が調整できるフローティングUIを追加。シニア（`preferredRole === 'senior'`）だけでなく全ロール・ゲストも使えるが、シニア選択時は**常時表示**・その他はフッター導線から開く設計。<br>①`components/layout/FontSizeControl.tsx` 新規：画面右下の丸型トグルボタン（A−/A / A+ アイコン）で 標準(1.0x) → 大(1.15x) → 特大(1.30x) の3段階を循環。<br>②表示条件：(a)`session.user.preferredRole === 'senior'` なら `/learn` と `/learn/[slug]` に固定表示、(b) その他のロール・未ログインは `Footer.tsx` に小さい「文字サイズ」リンクを出して押下で同じUIをモーダル表示。<br>③永続化：`localStorage.setItem('familyai:fontScale', ...)` でクライアント側のみ保持。ログイン時は将来 `users.preferences.fontScale` に同期する余地を残す（DB スキーマ追加は Phase 2 で別タスク化）。<br>④適用：`app/globals.css` に `:root { --font-scale: 1; }` を追加し、既存の `--text-body: clamp(14px, 3.5vw, 17px)` 等を `calc(clamp(14px, 3.5vw, 17px) * var(--font-scale))` に書き換え。見出し（`--text-title`/`--text-hero`）・本文（`--text-body`）・キャプション（`--text-caption`）すべてに適用。`/admin` 配下は除外（`app/admin/layout.tsx` で `--font-scale: 1` を再定義し固定）。<br>⑤a11y：ボタンは min 48×48px、`aria-label="文字サイズを変更"`、`aria-live="polite"` で状態変更を読み上げ、変更時に「文字サイズ: 大」等のトースト2秒表示。`prefers-reduced-motion` 時はトランジション無効。<br>⑥モバイル連携：`shared/constants/index.ts` に `FONT_SCALE = { small: 1.0, medium: 1.15, large: 1.30 } as const` を追加し、Web のみ適用（iOS/Android は OS の Dynamic Type を優先し、`Platform.OS === 'web'` でガード）。<br>⑦テスト：(a)`test/unit/fontScale.test.ts` で `cycleFontScale(1.0) === 1.15` 等のユニットテスト3件、(b) スモークテストに `GET /learn` レスポンスで `--font-scale` 変数が存在することを追加、(c) QA-T4 で「A+ を3回押して標準に戻る」E2E シナリオ追加。<br>**オープン事項（junli 確認事項）**: (i) トグルの段階数は 3段階 で OK か、もっと細かくするか（例: 100%/120%/140%/160% の 4段階）。(ii) 「特大」時に行間（`line-height`）も拡張するか。(iii) シニア以外のロールでもフッターから開けるのは OK か、それともシニアロール選択者限定にするか。** | CodingAgent | Phase 2 着手時 | 約120分 | — | ⏳未着手（ドラフト） |
| **Rev29 🟢（Claude コードレビュー検証結果・2026-04-25／実作業≒0・全件処理済み）: 私（CodingAgent / Claude）が `engineering:code-review` スキル + 4並列レビュー（Security/Performance/Correctness/Maintainability）で抽出した Critical 7件を別 Agent に検証してもらった結果、**①②③⑤の4件は False Positive（commit `1d1d7fc` で既に対応済みだったが、私の並列レビュー Agent が古い HEAD を見ていて再 flag してしまった）／④⑥の2件は真の問題で別 Agent が commit `14df190` で修正済み／⑦は記事数が少ない現段階では後回し妥当** と確定。<br>① **C1 register CSRF — False Positive ❌（既に commit `1d1d7fc` で実装済み）**：`app/api/auth/register/route.ts:38-44` に `verifyCsrf(req)` 配置済。<br>② **C2 register Rate Limit — False Positive ❌（同 `1d1d7fc`）**：同ファイル L46-54 で `getRateLimiter('ratelimit:register', 5, '10 m')`（5req/10min per IP）。<br>③ **C3 `/api/admin/users` ILIKE escapeLike — False Positive ❌（同 `1d1d7fc`）**：`app/api/admin/users/route.ts:17,39-40` で email/name 両方に `escapeLike()` 適用済。<br>④ **C4 `lib/csrf.ts` localhost bypass — ✅ 修正完了（commit `14df190`）**：`originHost === 'localhost' \|\| originHost.startsWith('localhost:') \|\| originHost === '127.0.0.1' \|\| originHost.startsWith('127.0.0.1:')` に修正。`localhost.attacker.com` バイパス防止。<br>⑤ **C5 admin/articles `pageSize` → `perPage` — False Positive ❌（同 `1d1d7fc`）**：`app/api/admin/articles/route.ts:42` で `perPage: result.pageSize` 既に正しい契約。<br>⑥ **C6 `shared/types` audio コメント削除 — ✅ 修正完了（commit `14df190`）**：`body: string;   // Markdown` に変更（audio 言及削除）。`drizzle/0006_remove_audio_fields.sql` のコミット要否は本番 `audio_play_logs` データ確認後に別途判断。<br>⑦ **C7 `getRelatedArticles` の `ORDER BY random()` — ✅ Rev25 へ合流（2026-04-25 junli 決裁）**：記事数が当面 5,000 件未満の現段階では実害なし。記事10,000件規模到達時に Rev25 と同時対応する方針で確定。対応詳細は Rev25 行の④に転記済み。<br>**🔍 教訓（運用ルール追加）**: 並列レビュー Agent（Explore 系）は古い HEAD ビューを参照することがある。`engineering:code-review` を走らせる前に必ず `git log --oneline -10` で直近のコミットを確認し、必要に応じて `git pull` を済ませてから実行すること。Critical 報告時は当該 commit の hash を併記すると検証コストが下がる。<br>**残・Rev30 候補（中優先度 Suggestion 5件・約90分）**: M1 register パスワードポリシー強化（8字 → 10字 + 特殊文字）／M2 admin 権限の DB 照合化（JWT のみだと剥奪不可）／M6 ISR revalidate 統一（300s）／M7 カテゴリ複数指定時の OR 句を `&&` 演算子に統合／M8 admin/articles バリデーション失敗時の `{ok,error}` ラッパー統一。<br>**残・Rev31 候補（A11y 改善 5件・約60分）**: M13 `confirm()` → モーダルダイアログ／M14 toggle ボタンの `aria-label`／M15 ArticleCard カテゴリ絵文字 `aria-hidden`／M16 ArticleForm エラーバナーの `aria-live`／M17 MobileNav フォーカストラップ。 | CodingAgent / 別 Agent | 2026-04-25 | 実作業≒0 | `1d1d7fc` / `14df190` | ✅完了（C1-C6）／⏳C7 後回し |
| **Rev32 🟢（Direct API 直接呼び出し対応 — DeepSeek / Qwen 公式 API／2026-05-07／約 90 分／実装完了）: OpenRouter のみだったAI呼び出しに、プロバイダー公式 API を直叩きする経路を追加。OpenRouter のマージン分コストを削減し、共有プール 429 リスクからも独立する。<br><br>🆕 **新規ファイル**:<br>① `lib/ai/provider-registry.ts` — プロバイダー（`openrouter` / `deepseek` / `qwen`）の `baseUrl` / `envKey` / 追加ヘッダーを集約した薄いレジストリ。`isProviderConfigured()` でキー有無を判定可能（管理画面の disabled 表示用）。<br>② `lib/ai/providers/openai-compatible.ts` — DeepSeek 公式 API・Qwen DashScope（OpenAI 互換モード）等の OpenAI 互換 endpoint を統一インターフェースで叩く汎用アダプタ。`completeOpenAICompat` / `streamOpenAICompat`。SSE → familyai 形式変換は OpenRouter と共通実装。<br><br>🔄 **既存ファイル変更**:<br>③ `shared/constants/ai-models.ts` — `AiModelOption` に `provider: 'openrouter' \| 'deepseek' \| 'qwen'` と `nativeId?: string` を追加。既存 5 モデルは `provider: 'openrouter'` を付与（破壊的変更なし）。新規 5 モデル登録: `deepseek:deepseek-chat`（V3 Chat）/ `deepseek:deepseek-reasoner`（R1）/ `qwen:qwen-turbo` / `qwen:qwen-plus` / `qwen:qwen-max`。料金は USD ベース ¥150 換算で AI_MODEL_OPTIONS に登録（DeepSeek-V3 ¥41/¥165 /M、Qwen-Plus ¥17/¥50 /M 等）。<br>④ `lib/ai/router.ts` — `streamByModelId` / `completeByModelId` / `completeByModelIdWithUsage` の 3 関数を追加。`findAiModel(id).provider` で OpenRouter or 直 API を分岐。未登録 ID は OpenRouter フォールバック（後方互換）。`routeAI()` の中身も新 dispatcher に置換。<br>⑤ `app/api/generate-animation/route.ts` — Stage1/Stage2/Legacy fallback の 3 callsite を `completeOpenRouter*` → `completeByModelId*` に切替え。コードの他の部分は無変更。<br>⑥ `lib/env.ts` — `DEEPSEEK_API_KEY` / `DEEPSEEK_BASE_URL` / `DASHSCOPE_API_KEY` / `DASHSCOPE_BASE_URL` を任意 env として追加。<br>⑦ `app/admin/ai-config/parts.tsx` — `ModelSelect` を provider 別 `<optgroup>` 化（OpenRouter / DeepSeek 公式 / Qwen 公式 の 3 グループ）。<br>⑧ `app/admin/ai-config/SettingsTable.tsx` — モデル下のヒント行に provider バッジ（OpenRouter 紫 / DeepSeek 緑 / Qwen 黄）を表示し、選択中モデルの呼び出し経路が一目でわかるように。<br><br>🧪 **テスト**: `test/unit/aiProviderRegistry.test.ts` 新設（13 ケース）。`getProviderConfig` / `isProviderConfigured` / `listProviderStatus` / `findAiModel` / DeepSeek / Qwen モデル登録の検証。`vi.resetModules` で `DEEPSEEK_BASE_URL` / `DASHSCOPE_BASE_URL` 環境変数の上書き挙動も網羅。<br><br>🔐 **junli 環境での運用準備**:<br>(i) Vercel Project Settings → Environment Variables に `DEEPSEEK_API_KEY`（DeepSeek プラットフォーム取得）と `DASHSCOPE_API_KEY`（Alibaba Cloud DashScope 取得）を追加。両方とも optional（未設定なら当該 provider の選択時にエラー文言で気づける設計）。<br>(ii) 中国大陸内サーバから叩く場合は `DASHSCOPE_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1` を設定。デフォルトは国際版（`dashscope-intl.aliyuncs.com`）。<br>(iii) AI教室 Stage2 で Anthropic prompt cache を活用したい場合は OpenRouter 経由のままが有利（`completeByModelIdWithUsage` は直 API では cache 統計を返さない）。<br><br>**検証ログ**: `pnpm exec tsc --noEmit` EXIT=0 / `pnpm lint` 0 件 / `pnpm test` 17 files・159 tests PASS（+13 件）。<br><br>**残・将来候補（Rev32+）**: (a) Anthropic 公式 API ネイティブ adapter（prompt cache を直 API で利用したい場合のみ）／(b) OpenAI 公式 / Gemini 公式 / Groq 直 API（同じ openai-compatible.ts で対応可能・モデル登録だけで完了）／(c) provider 別の月間コスト集計と alert（Stripe 課金導入後）。** | CodingAgent | 2026-05-07 | 約 90 分 | — | ✅完了 |
| **Rev31 🔴（Codex レビュー対応 + Phase 1 ビルド安全性 + Phase 3 安定性インフラ + Phase 4 a11y ／2026-05-05 起票・2026-05-07 で Phase 1+3+4 同時完了）: 外部 AI Agent (Codex) によるコードレビュー19件と Claude Code Option A レビューの結果を統合。19/19 全件 True Positive 確認。優先度別に Phase 1〜5 で段階対応した。Phase 2 のみ junli 環境での実装に持ち越し。<br><br>🟢 **Phase 1（完了・commit `754396a`）— ビルド安全性／約30分**<br>① **lock file 統一**: `package-lock.json` を削除（npm/pnpm 二重 lockfile による dev/prod drift を解消）。`pnpm-lock.yaml` 一本化。<br>② **packageManager pin**: `package.json` に `"packageManager": "pnpm@9.15.0"` と `"engines": { "node": ">=20" }` を追加（Corepack で再現性確保）。<br>③ **CI ビルドジョブ追加**: `.github/workflows/test.yml` に `build` ジョブ（`needs: static-and-unit`）を新設。`pnpm install --frozen-lockfile` + `pnpm build` をダミー env（`DATABASE_URL`/`NEXTAUTH_SECRET`/`NEXTAUTH_URL`）で実行し、依存ドリフト・Next.js config エラー・モジュール解決失敗を PR 時に検出（CX-6 対応）。実 secrets が要る `smoke` ジョブはコメントアウト維持。<br>④ **検証**: `pnpm build` ローカル検証は次回 fresh `pnpm install` 完走後に実施。CI で初回押下時に検証される。<br><br>🔴 **Phase 2（⏳junli 環境での再実行待ち・約2〜3時間）— セキュリティアップグレード**<br>※ 2026-05-07 セッションでは Claude のローカル `pnpm add` がネットワーク資源解決でハングしたため未完了。junli 環境で `pnpm add next-auth@5.0.0-beta.31` 単独実行 → 動作確認 → 別 PR で `pnpm add next@15.5.16 eslint-config-next@15.5.16 + npx @next/codemod@canary upgrade latest` の順を強く推奨。<br>**🆘 Phase 2-aux（フォント DL 不安定対応・本コミットに含む）**: ローカル `pnpm build` で `next/font/google` が `fonts.gstatic.com` から日本語フォント subset を数百ファイル並列 DL する際、断続的なネットワーク失敗で build 全体が落ちる事象を確認。本セッションで以下の対処を実装:<br>  (i) `scripts/build-retry.sh` を新設（最大 10 回 build 再試行・成功した font shard は `.next/cache` 経由で次回再利用）。<br>  (ii) `app/layout.tsx` の Zen Kaku Gothic から未使用の `weight: '900'` を削除（`grep -r font-weight` で実コード参照ゼロ確認）。約 14% の DL ファイル数削減。<br>  (iii) **恒久対応として推奨**: junli 環境で `pnpm add @fontsource/zen-kaku-gothic-new @fontsource/shippori-mincho` 後、`app/globals.css` 冒頭で必要 weight の CSS を `@import` し、`app/layout.tsx` の `next/font/google` 呼び出しを撤去すれば build 時のリモート DL がゼロになる（npm 配信の woff2 が node_modules に同梱されるため）。<br>⑤ **Next.js 14.2.35 → 15.x アップグレード**: `pnpm audit` で 2件 HIGH CVE 検出（HTTP request deserialization DoS / Server Components DoS、いずれも 14.x line では fix 提供なし・15.0.8+ / 15.5.15+ で修正）。**Breaking change**: `params` / `searchParams` が `Promise` 化、`fetch` キャッシュデフォルト変更、Sharp 必須化など。対応手順:<br>  - `pnpm add next@latest eslint-config-next@latest`<br>  - `npx @next/codemod@canary upgrade latest` で `params` / `cookies()` / `headers()` の自動 Promise 化<br>  - 全 `app/(site)/learn/[slug]/page.tsx` 等の dynamic route で `params: Promise<{slug:string}>` への型修正<br>  - `pnpm build` + ローカル smoke + Vercel preview で全フロー検証（記事閲覧・AI 生成・admin CRUD・auth signin）<br>⑥ **NextAuth 5.0.0-beta.25 → 5.0.0-beta.31 マイナー更新**: 同 minor 系で安全。`pnpm add next-auth@5.0.0-beta.31`。Session/JWT callback 影響軽微だが `lib/auth.ts` のサインインフローを smoke テスト。<br>⑦ **依存最新化チェック**: `pnpm audit --prod` を CI に追加検討（false-positive 抑制のため `--ignore-registry-errors`）。<br><br>🟢 **Phase 3（完了・commit `c7dc44a`）— 安定性インフラ**<br>⑧ **CX-1 Suspense ローカライズ**: `app/(site)/learn/page.tsx` 等の `<Suspense fallback={<div>Loading...</div>}>` を「読み込み中…」に統一。`shared/constants/index.ts` に `LOADING_FALLBACK = '読み込み中…'` 追加し全箇所で参照。<br>⑨ **CX-2 SSE setMessages スロットリング**: `components/article/AIChatWidget.tsx` の SSE トークン受信で React state 更新を 50ms throttle（lodash 不要・自前 `useThrottle` hook）。長文応答時のジャンクを解消。<br>⑩ **CX-4 ai-memos / vocab-bookmarks ページネーション**: `app/api/user/ai-memos/route.ts` と `app/api/user/vocab-bookmarks/route.ts` に `page` / `pageSize`（デフォルト 50・上限 200）を `lib/schemas/users.ts` 同形式の zod で導入。レスポンスを `{ items, meta: {total, page, perPage} }` に統一（admin/articles と同契約）。クライアント `shared/api/index.ts` の `fetchUserAiMemos` / `fetchUserVocabBookmarks` も meta 受け取りに拡張。<br>⑪ **CX-5 vocab bulk 上限縮小**: `app/api/user/vocab-bookmarks/bulk/route.ts` の許容件数 2000 → 300 に変更（DoS / Neon row insert タイムアウト対策）。zod 上限値とエラーメッセージ「最大 300 件まで一度に保存できます」更新。<br>⑫ **CX-7 構造化ロギング**: `lib/log.ts` 新規作成。`logger.info/warn/error(label, meta)` で JSON 形式（`{ ts, level, label, ...meta }`）出力。`route.ts` 全 17 箇所の `console.error` / `console.warn` を順次置換（最低限 admin / ai 系を優先）。Vercel ログでクエリ可能化。<br>⑬ **CX-8 env validation (`lib/env.ts`)**: zod `serverEnvSchema` で `DATABASE_URL` / `NEXTAUTH_SECRET` / `OPENROUTER_API_KEY` / `UPSTASH_REDIS_REST_URL` 等を起動時検証。`.parse(process.env)` 失敗時は明確なエラーで起動拒否。`lib/db/index.ts` lazy パターンと両立させる（client 側だけ tolerant）。<br>⑭ **CX-9 ai-config degraded 表示 + `/api/health`**: `/admin/ai-config` で「DB 取得失敗 → DEFAULTS 動作中」を視覚バッジで提示。`app/api/health/route.ts` 新規（`{ db: 'ok'/'fail', ai: 'ok'/'fail', ts }` を返す軽量エンドポイント・公開可）。Vercel Cron 後段の死活監視として活用。<br><br>🟢 **Phase 4（完了・commit `c7dc44a`）— UX / アクセシビリティ**<br>⑮ **CX-3 alert/confirm 統合（17箇所）**: `components/ui/ConfirmDialog.tsx` / `useConfirm` hook を新規。全 `window.confirm()` / `window.alert()` を Base UI Dialog ベースに置換。シニア配慮（Rev28 連動）でフォントサイズ変数 (`--font-scale`) も尊重。<br>⑯ **CX-10 `<button>` 内 anchor 構造修正**: HTML invalid な `<button><Link>...</Link></button>` を全 grep で検出して `<Link className="...">` 単体に統一（screen reader / W3C validator 警告解消）。<br>⑰ **CX-11 toggle ボタン aria-label**: `AdminArticleTable.tsx` 公開トグル等に「公開を切り替え（現在: 公開中）」など状態を含む `aria-label` を付与。<br>⑱ **CX-12 ArticleCard 絵文字 aria-hidden**: カテゴリアイコン等の装飾絵文字を `<span aria-hidden="true">📰</span>` でラップ（screen reader が「絵文字 ニュースペーパー…」と読み上げる現象解消）。<br>⑲ **CX-13 MobileNav フォーカストラップ**: メニュー開時に `inert` 属性 + Tab 巡回で背後へフォーカスが抜けないよう Base UI Dialog の focusManager パターンで実装。<br><br>🔵 **Phase 5（後日・記事数依存）— Option A レビュー残（A11-1 / A11-2）**<br>⑳ **A11-1 バンドル最適化**: `next-bundle-analyzer` で 100KB 超のクライアントコンポーネントを特定し、`dynamic()` import / Server Component 化で削減。<br>㉑ **A11-2 AI教室 1764 行ファイル分割**: `app/(site)/ai-kyoshitsu/page.tsx`（仮）等の 1000 行超ファイルを feature ディレクトリに分割（`components/ai-kyoshitsu/{Form,Result,History}.tsx` 等）。<br><br>📋 **検証戦略**:<br>- Phase 2-4 は各々独立 PR で着手し、CI build job + Vitest 全件 PASS + Vercel preview smoke を必須ゲートとする。<br>- Phase 2 (Next.js 15) は単独 PR で `pnpm test` + `pnpm build` + `vercel build --prod` ローカル + preview deploy 全 4 ゲート通過必須。<br>- Phase 3 CX-7/8 (logging/env) は同一 PR でまとめると config 影響範囲が局所化できる。<br><br>**残 junli 確認事項**: (i) Phase 2 着手は MVP 5/8 リリース前？後？（リリース前推奨だが breaking change リスクあり）／(ii) CX-9 `/api/health` を Vercel Cron で監視するか、外部 (UptimeRobot 等) に任せるか／(iii) Phase 4 a11y は Rev28 (font scale) と統合 PR にするか別建てか。** | CodingAgent | Phase 1 ✅ 2026-05-05 ／ Phase 2 リリース前後 ／ Phase 3 リリース後2週 ／ Phase 4-5 順次 | 合計 約9〜12時間 | Codex review (`todo/20260507_Codexレビュー.md`) ／ pnpm audit ／ Option A review | 🟢 Phase 1 着手済 ／ Phase 2-5 ⏳未着手 |
| **Rev30 🟠（AI教室パイプライン本実装＋管理画面＋コードレビュー対応／合計約12時間／2026-04-25〜2026-04-28）: 「うごくAI教室」機能（学年×教科×テーマからアニメーションHTMLを生成する2段階パイプライン）の本実装と、運用中にモデル/タイムアウト/トークン上限を切り替えられる管理画面、および Rev30 コードレビュー対応をまとめて実施。<br>🆕 **AI教室パイプライン（合計約6時間）**<br>① **2段階パイプライン**：`app/api/generate-animation/route.ts`（POST）で Stage1（JSON設計）→ Stage2（HTML生成）。Stage1 は `skills/ai-kyoushitsu-prompt/stage1_system_prompt.md` を system に、ユーザ入力（学年/教科/テーマ）を user として渡し、教育設計 JSON or 確認質問 or エラーJSON のいずれかを返す（zod で 3種類のスキーマ判定）。Stage2 は構造化 JSON を受け取り `stage2_SKILL.md` に従い HTML 出力（プロンプトキャッシュ対応・`cache_control: { type: 'ephemeral' }`）。Stage1 が parse_failed なら `legacy_unified.md` で legacy フォールバック。<br>② **shared/types**: `AnimationGrade` / `AnimationSubject` / `AnimationSummary` / `Animation` / `AiKyoshitsuConfig` を追加（iOS/Android 移植対応の pure TypeScript）。<br>③ **DB**：`drizzle/0009_user_animations.sql` で `user_animations` テーブル（id/user_id/theme/grade/subject/prompt/html_content/created_at）。`lib/repositories/animations.ts` に `createAnimation` / `getAnimationByIdCached`（React.cache）/`listUserAnimations`（最大50件）を実装。<br>④ **API公開**：`GET /api/user/animations`（履歴一覧・認証必須・htmlContent抜きの AnimationSummary[]）と `GET /api/animations/[id]`（単体・認証なしで shared 用）を新設。`shared/api/index.ts` に `fetchUserAnimations` を追加。<br>⑤ **共有ページ**：`/share/[id]` の SSR ページで X / LINE / コピーボタンを実装。OGP は `app/share/[id]/opengraph-image.tsx` で動的生成。<br>⑥ **レート制限**：未ログイン=利用不可（401）／Free=3回/日／Premium=100回/日／Admin=無制限。Upstash Redis の slidingWindow を使用。<br>⑦ **Vercel 60秒制限対策**：Stage1 + Stage2 タイムアウト合計を 50秒以下に保つ。プロンプトキャッシュで Stage2 入力を 80% 以上削減。`next.config.mjs` の `outputFileTracingIncludes` で `skills/ai-kyoushitsu-prompt/*.md` を bundle に含める。<br>🆕 **管理画面（合計約3時間・Phase 1+2）**<br>⑧ **DB**：`drizzle/0010_ai_config.sql` で `ai_config`（1行限定 id=1・jsonb config）と `ai_config_history`（直近10件・changed_at index）。`lib/repositories/ai-config.ts` に CRUD と pruneHistory（drizzle `notInArray` 使用）。<br>⑨ **設定マージ層**：`lib/config/ai-config.ts:getAiConfig()` で 「DEFAULTS < DB < env」の3層を 60秒キャッシュで解決。PUT 時に `invalidateAiConfigCache()` で即時反映。<br>⑩ **管理画面 UI**：`/admin/ai-config` Server Component が effective+dbPartial+history を取得し `AiConfigForm`（Client）に渡す。プリセット切替・コスト試算・履歴表示・保存（PUT）・リセット（DELETE）を備える。`shared/constants/ai-models.ts` に `AI_MODEL_OPTIONS`（5モデル・料金テーブル）と `AI_CONFIG_PRESETS` と `AI_CONFIG_RANGES`（zod 範囲検証用）を集約。<br>⑪ **API**：`app/api/admin/ai-config/route.ts` GET/PUT/DELETE。zod で `AI_CONFIG_RANGES` を強制し、未知のモデル ID や範囲外を 400 で返却。CSRF + `isAdmin` の二重ガード。<br>⑫ **AIチャットも `cfg.chatModel` を尊重**：`lib/ai/router.ts` を更新し、cfg.chatModel が DEFAULTS と異なる場合は OpenRouter MODEL_ROUTER より優先。<br>🆕 **「🛡️ 安定運用」プリセット追加（2026-04-28）**：Anthropic Claude Haiku 3.5 を全Stageで使う構成を最先頭プリセットに追加。Gemini 2.0 Flash 共有プールの 429 / AbortError 多発を回避。timeout=12s/40s・maxTokens=4000・cost ≒ 3.6円/回。既存「最安構成」「バランス」は Gemini ベースとして警告を追記、「品質重視」は Stage2 のみ Haiku で残置。<br>🟠 **Rev30 コードレビュー対応（C1/C2/C4 + S4/S7/S11／合計約2時間）**<br>**C1 admin/users zod**：`lib/schemas/users.ts` 新規（`adminUsersQuerySchema` で page/pageSize/search/plan/sort を coerce + 範囲検証）→ `app/api/admin/users/route.ts` で `safeParse → 400`。NaN が offset/limit に流入するのを防止。<br>**C2 shared/types**：`AiMemoItem` / `VocabItem` を `shared/types/index.ts` に追加し、`lib/ai-memo-store.ts` と `lib/voaenglish/vocab-store.ts` は shared 型を re-export する形に変更（iOS/Android で型共有可能に）。<br>**C4 mapper パターン徹底**：`lib/mappers/ai-memos.ts`（`toAiMemoItem`）と `lib/mappers/vocab-bookmarks.ts`（`toVocabItem`）を新設し、`/api/user/ai-memos` `/api/user/vocab-bookmarks` の DB row→DTO 変換をインライン記述から mapper 経由に統一。`shared/api/index.ts` に `fetchUserAiMemos` / `fetchUserVocabBookmarks` を追加。<br>**S4 dynamic→static**：`lib/repositories/ai-config.ts` の `notInArray` を dynamic import から static import に変更（バンドル静的解析しやすく）。<br>**S7 deprecated 削除**：`shared/constants/index.ts` から旧 `AI_MODELS` 定数を削除（MODEL_ROUTER に統一済み）。<br>**S11 console.error スタックトレース保護**：route.ts の `console.error/warn` 4箇所を `err instanceof Error ? err.message : String(err)` に変更（Vercel ログのスタックトレース漏れと PII 混入を抑制）。<br>🟡 **H4 AiConfigForm 分割（2026-04-28・約45分）**：422 行の `AiConfigForm.tsx` を 6 ファイルに分割。<br>  - `parts.tsx`（共通 UI 部品 Section/Field/Hint/Stat/ModelSelect/NumberInput + styles）<br>  - `PresetSwitcher.tsx`（🎁 プリセット切替）<br>  - `StageFields.tsx`（🧠 Stage1 / 🎬 Stage2 / 💬 Chat フィールド）<br>  - `CostEstimator.tsx`（💰 コスト試算 + 月間試算）<br>  - `HistoryList.tsx`（📜 変更履歴）<br>  - `AiConfigForm.tsx`（薄いコンテナ・状態管理 + 保存controls）<br>各ファイル 50〜170 行に収まり、保守性向上。<br>🟡 **T1 admin/ai-config 統合テスト追加（2026-04-28・約45分）**：`test/integration/admin-ai-config.integration.test.ts` 新規（15件）。`vi.mock('@/lib/auth')`/`@/lib/repositories/ai-config'`/`@/lib/config/ai-config'` で Route Handler を直接呼び出し。<br>  - GET /api/admin/ai-config: 200 + effective/dbPartial／403 非管理者／403 未ログイン／500 DB 例外<br>  - PUT: 200 + saveAiConfig 引数検証 + invalidateCache／403 CSRF／403 非管理者／400 不正 JSON／400 未知モデル／400 範囲外／500 DB 例外<br>  - DELETE: 200 + resetAiConfig 引数検証／403 CSRF／403 非管理者／500 DB 例外<br>**検証ログ（統合）**: `pnpm exec tsc --noEmit` → EXIT=0／`pnpm lint` → 0件／`pnpm test` → Test Files 10 passed・Tests 83 passed (68→83・+15)。<br>**残タスク**: H2 管理者ロール DB 化（junli判断で不要・ADMIN_EMAIL 1人運用継続）／H3 監査ログ（不要）／H5 legacy_unified.md cache_control（不要・Stage1 success ルートでのみキャッシュ効くため・skip）。<br>**⚠️ R3-U2 保留中の課題（2026-04-28）**: `app/(site)/share/[id]/opengraph-image.tsx` を追加したが、ローカル dev で `/share/[id]/opengraph-image` 直接アクセス時に 404 が返る事象を確認。Edge runtime → Node.js runtime 変更でも解消せず。原因候補は (1) route group `(site)` 配下に opengraph-image 規約ファイルを置いた時の dev server 認識問題、(2) `getAnimationByIdCached` の dev での例外、(3) `.next` キャッシュ起因のいずれか。対策候補は (A) `app/share/[id]/opengraph-image.tsx`（route group 外）への移動、(B) `rm -rf .next` 後の再検証、(C) Vercel 本番ビルドでの動作可否確認（本番では Edge/Node のサポートが dev と異なる可能性）。コミット bbfc9f8 にファイル自体は含めたまま保留。本番デプロイ時に動作するなら閉じる、ダメなら次セッションで原因特定して修正。<br>**⚠️ R2-O1/O2 保留中**: Sentry 導入（O1）・Vercel Speed Insights 導入（O2）はネットワーク不安定（pnpm add 中の ECONNRESET 多発）で 2026-04-28 セッションでは中断。次回ネットワーク回復時に `pnpm add @sentry/nextjs @vercel/speed-insights` から着手。<br>**本番DB マイグレーション**: `drizzle/0009_user_animations.sql` `0010_ai_config.sql` ともに 2026-04-28 時点で本番 Neon に適用済み（`familyai.jp/admin/ai-config` 動作確認）。** | CodingAgent | 2026-04-28 | 約720分（12時間） | — | ✅完了（C1-C4・S4/S7/S11・H4・T1・H6 / H2/H3/H5 は不要判断） |
| **Rev36 🟠（うごくAI教室 3D 図鑑化・Phase 1 / 段階 1-3 = 2026-05-11 着手 ／ 段階 4 = junli さん確認後）: 「うごくAI教室」を AI 生成アニメーション機能から、厳選 3D モデル閲覧型「3D 図鑑」へ全面リプレイス。Phase 1（本 Rev）はカタログ閲覧のみ・Phase 2 で AI 3D 生成（Tripo API）を後付け。URL `/tools/ai-kyoshitsu` 維持・ブランド名「うごくAI教室」維持・段階公開 `adminOnly: true` で開始。<br><br>🆕 **コンテンツ方針**：理科のみ・4 サブカテゴリ（生物 / 化学 / 地学宇宙 / 物理）。Phase 1 ローンチは 3 モデル（オール AI 生成方針）：🪐 太陽系（地学宇宙）・🧬 DNA 二重らせん（化学）・⚡ 振り子の運動（物理）。junli さんが Blender Python スクリプト経由でローカル生成 → Vercel Blob に配置。Phase 2 で 10 モデルに拡充（偏り重視：生物 5 / 宇宙 3 / 物理 1 / 化学 1）。<br><br>🆕 **UI コンセプト「ぬくもりキャビネット」**：cream→peach の柔らかい円形スポットライトに 3D モデルを浮かべ、既存「あいちゃん」が横で語りかける構成。pill 型ボタン（🔍ズーム / 🌐AR / ⭐保存 / 💬チャット）を画面下部に配置。スマホ縦表示優先（既存方針継承）。<br><br>🗑️ **削除（旧資産・α方針＝完全削除）**：`app/api/generate-animation/` ／ `app/api/animations/` ／ `lib/ai-kyoshitsu/*`（stage1-schema.ts ／ themes.ts ／ conversation.ts ／ legacy_unified.md など全部）／ `skills/ai-kyoushitsu-prompt/stage*.md` ／ `stage2_SKILL.md` を全削除。`app/(site)/tools/ai-kyoshitsu/page.tsx` も完全書き換え。`user_animations` テーブルの既存データは全 DELETE。Phase 2 で AI 生成版を作る際は git 履歴から参照する方針。<br><br>🔄 **DB マイグレーション 0018**：(1)`user_animations` の `html_content` を NULL 許可に変更し、`glb_url text` ／ `usdz_url text` ／ `thumbnail_url text` ／ `hotspots jsonb` カラムを追加（既存 row は事前に全削除）。(2) 新規 `user_3d_bookmarks` テーブル（`user_id` ／ `model_id` ／ `created_at` ・複合 unique）。(3) 新規 `tutor3d_models` テーブル（admin 管理・`slug` unique ／ `title` ／ `description` ／ `subject` enum (biology/chemistry/earth-space/physics) ／ `grade` enum (elem-low/elem-high/middle) ／ `glb_url` ／ `usdz_url` ／ `thumbnail_url` ／ `hotspots jsonb` ／ `attribution` ／ `license` ／ `source_url` ／ `published` bool ／ `is_featured` bool ／ `view_count` int）。indexes は `tutor3d_models(subject)`、`tutor3d_models(published, is_featured)`、`user_3d_bookmarks(user_id, created_at)` を含める。<br><br>🔄 **API 改修**：(a)`/api/generate-animation` → 削除。(b)`/api/animations/[id]` → 削除し `/api/3d-models/[slug]` 新設（GLB URL ／ hotspots ／ metadata を返却・既存 `enforceXxxRateLimit` パターン適用で prod fail-closed・Rev31-P1 準拠）。(c)`/api/user/animations` → 削除し `/api/user/3d-bookmarks` 新設（GET 一覧 ／ POST 追加 ／ DELETE）。(d)`/api/admin/3d-models` 新設（CRUD・既存 `/api/admin/articles` パターン踏襲・CSRF + `isAdmin` 二重ガード）。(e) ホットスポット深掘りチャットは既存 `/api/ai` をそのまま流用（system prompt にモデル情報・タップされた hotspot 情報を埋め込む）。<br><br>🔄 **フロント書き換え**：(i)`app/(site)/tools/ai-kyoshitsu/page.tsx` をカタログ一覧に書き換え（サブカテゴリ × Grade フィルタ）。(ii)`app/(site)/tools/ai-kyoshitsu/[slug]/page.tsx` 新設（個別モデルページ・`<model-viewer>` を `dynamic(() => ..., { ssr: false })` で読み込み・`detect-gpu` で低スペック端末フォールバック）。(iii)`app/share/[id]/page.tsx` を **iframe HTML → `<model-viewer>` 直接レンダリングに置換**（iframe 非使用なので Rev31-P0 の CSP/sandbox 強化と非競合・むしろ干渉ゼロ）。(iv)`components/tools/3d-tutor/` 配下に ModelGallery ／ ModelViewer ／ ModelHotspot ／ AITutorChat ／ ARButton ／ ModelMetadata ／ ModelLowEndFallback を配置。(v)`app/admin/3d-models/` 新設（CRUD UI + hotspot エディタ）。<br><br>📦 **依存パッケージ追加**：`@google/model-viewer`（Web Component）・`three`（peerDependency）・`detect-gpu`（低スペック端末フォールバック）。<br><br>🔐 **公開モード**：既存 `ai-kyoshitsu` と同じく `adminOnly: true` で段階公開開始（`tools/page.tsx:56` の既存 Rev33 パターン継承）。junli さん + テスター限定で動作確認後、`adminOnly: false` に切替。<br><br>🌐 **AR モード**：MVP から `<model-viewer ar ar-modes="webxr scene-viewer quick-look">` で iOS Quick Look / Android Scene Viewer ネイティブ対応。USDZ ファイルは junli さん（Mac 環境）で Reality Converter で事前生成。Vercel Blob CORS 設定要確認（既存 audio 配信で動作実績あり）。<br><br>🛠️ **junli さん側ローカルツール（段階 2 で配布）**：`scripts/3d-tools/` 配下に (a)`preview.html` ローカル GLB プレビュー、(b)`compress.sh` gltf-pipeline Draco 圧縮、(c)`hotspot-picker.html` 座標採取、(d)`attribution-template.md` ／ `README.md` ／ `generate-solar-system.py` Blender Python スクリプト。junli さんは Blender 未経験でも CLI 1 行で 3D 生成可能。<br><br>🧪 **テスト戦略**：ユニット（hotspot 座標バリデーション・shared API contract）・統合（`/api/3d-models/[slug]` の 200/404・`/api/user/3d-bookmarks` CRUD・auth/rate limit）・スモーク（`GET /tools/ai-kyoshitsu` 200・`GET /api/3d-models/solar-system` 200）。E2E（QA-T4 Playwright）は Phase 2 着手前。<br><br>⏸ **Phase 2 候補（本 Rev には含まない）**：(α) AI 3D 生成モード（Tripo API・webhook + polling 方式・約 20h）。(β)`TRIPO_API_KEY` ／ `TRIPO_WEBHOOK_SECRET` を `lib/env.ts` zod schema に追加（Rev31-P2 env validation 準拠）。(γ) 旧 themes.ts 相当を 3D 向け 10 テーマで新規作成。(δ) 10 モデルに拡充（生物特化）。(ε) ジオラマモード・ふしぎ図鑑のスタンプ要素は Phase 3 候補。<br><br>🚦 **段階的実行**：段階 1 = 本 Rev 書き起こし（30分・本コミット）／段階 2 = `scripts/3d-tools/` ローカルツール 4 種作成（4〜5h）／段階 3 = AI で太陽系 GLB 試作（1〜2h）／⏸ junli さん確認 → 段階 4 = Next.js 本体実装（25〜30h）。段階 4 着手前に junli さんから明示的な GO サインを受領すること。<br><br>**前提・関連 Rev**：Rev28（文字サイズ）は本 Rev の `<model-viewer>` コンテナにも `var(--font-scale)` 経由で適用される設計。Rev31-P0 iframe sandbox 強化は `<model-viewer>` が iframe 非依存のため非競合。Rev31-P1 rate limit fail-closed は新規 API すべてで遵守。Rev31-P2 env validation は Phase 2 で `TRIPO_API_KEY` を追加する際に対応。マイグレ番号は 0018 から開始（0017 まで適用済み確認）。** | CodingAgent | 段階 1-3 = 2026-05-11 ／ 段階 4 = junli さん確認後 | 段階 1-3 ≒ 6h ／ 段階 4 ≒ 25-30h | Rev30 (削除対象) ／ Rev31-P0/P1 (security 非競合) ／ Rev28 (font scale 連動) | 🟢 段階 1 着手中 ／ ⏳段階 4 junli 確認待ち |
| **Rev39 🟠（運用安定化・log 構造化移行 + 3D orphan blob cleanup ／合計約180分／2026-05-13 実装・本番反映・後に自動 Cron 一時停止）**: Rev38 でスコープ外とした観測性向上 + cleanup ジョブを 2 Task で実装。<br>✅ **Task 1 (H5 構造化ログ移行)**: scope（`app/api/`・`lib/repositories/`・`lib/ai/`）の `console.error/warn` を **42 件 → 0 件**に完全移行。全 24 ファイルで `withRequest(req, route)` + `log.error/warn(label, meta)` パターンを統一適用。ラベル命名規約は `route.action` 形式（`admin.3d-models.list`・`ai-echo.post`・`articles.related`・`tts.post`・`3d-assets.blob_fetch_failed` 等）。スタックトレース流出防止のため `error.message` に絞る。<br>✅ **Task 2 (3D Orphan Blob Cleanup)**: `app/api/cron/cleanup-3d-orphans/route.ts` 新規（162 行・JSDoc 完備）+ `vercel.json` 新規（`schedule: "0 18 * * *"` = JST 03:00）+ `lib/env.ts` に `CRON_SECRET` 追加（optional・min 32 文字）。`@vercel/blob list()` を hasMore/cursor でページング全件回収・`urlToPathname()` で proxy/直 Blob URL 両形態を正規化・`del()` 失敗は個別 catch で集計。<br>✅ **本番動作確認（2026-05-13）**: Vercel 環境変数 `CRON_SECRET` 設定後、`curl -i https://www.familyai.jp/api/cron/cleanup-3d-orphans` で HTTP/2 200 + JSON 応答確認。**初回実行で Rev38 #H4 累積の orphan 2 件（約 7 MB）を即座に回収**。<br>⚠️ **自動 Cron は一時停止中（2026-05-13・junli 判断）**: 「誤削除リスクが怖い」という junli の懸念を尊重し、`vercel.json` を削除して Vercel Cron 登録を解除。**route 自体は残しているので curl で必要時に手動実行可能**。Rev40 で defensive 設計（CRON_DRY_RUN モード・grace period 1h・DB 参照 0 件時の中止・認証失敗時 IP/UA ログ）を追加してから `vercel.json` を再作成して自動 Cron を復活させる方針。<br>**検証ログ**: `tsc --noEmit` 0 ／ `pnpm lint` clean ／ `pnpm test` 18 files 164 tests passed ／ 本番 curl 200 ／ 実 orphan 削除 2 件成功。<br>**Rev40 で必須対応**: (i) `CRON_DRY_RUN=true` で削除をスキップしログのみ出力するモード追加・(ii) 直近 1h にアップロードされた Blob を cleanup 対象から除外（grace period）・(iii) DB 参照が 0 件の場合は sanity check で中止・(iv) 認証失敗時の IP/User-Agent ログ。安全装置完成後に `vercel.json` 再作成で自動 Cron 復活。<br>**残課題（Rev40 候補）**: (i) Sentry 導入（`@sentry/nextjs`）・(ii) クライアント側 `console.error` の telemetry 移行・(iii) Cron 失敗時の Slack/メール通知（`failedCount > 0` 時）・(iv) admin 内 OpenRouter コストダッシュボード・(v) **apex/www ドメイン統一**（Vercel で apex を primary に変更 or コード全体を www に統一・現在 canonical URL と実 URL が不一致で SEO 上の食い違いあり）。<br>**残課題（Rev41 候補）**: Playwright E2E（ログイン → 3D 図鑑閲覧 → AI Echo 履歴）+ Cron route の vitest 統合テスト。 | CodingAgent / 別 Agent | 2026-05-13 | 約180分 | Rev38 ✅ | ✅完了（本番反映済・自動 Cron は Rev40 完了後に復活） |
| **Rev38 🟠（rev36/37 自己レビュー対応・3D 図鑑機能+AI Echo の High 5件／合計約90分／2026-05-13 実装・本番反映完了）**: 私（CodingAgent / Claude）による 4 軸レビュー（Security/Performance/Correctness/Maintainability）で抽出した High 6 件のうち、影響範囲を限定できる 5 件を一括対応。H5（console.error → lib/log.ts 31 箇所移行）は範囲が広いため Rev39 として分離。<br>✅ **H1**: `app/api/user/ai-echo/route.ts` の GET/POST レスポンスを `apiFetch` 契約準拠（`{ok, data}`）に統一（破壊的変更・iOS Client 着手前ブロッカー解消）。<br>✅ **H2**: `drizzle/0020_tutor3d_models_published_slug_idx.sql` 新規 + `lib/db/schema.ts` に `idxPublishedSlug` 追加。本番は Neon Console SQL Editor で `CREATE INDEX CONCURRENTLY` 発行済み（2026-05-13 確認・6 rows 反映）。<br>✅ **H3**: `lib/repositories/3d-models.ts:togglePublishedBySlug` を SELECT → JS 反転 → UPDATE の 2 段階から、`SET published = NOT published + RETURNING` の 1 文 atomic 化。並行 PATCH の race condition 解消。<br>✅ **H4**: `app/api/admin/3d-models/[slug]/route.ts:DELETE` の処理順を「DB 削除先 → Blob 削除後」に変更。Blob 失敗は warn-only で 200 返却（orphan blob は Rev39 cleanup ジョブで対応）。<br>✅ **H6**: `lib/repositories/3d-models.ts:listAllModelsForAdmin` の try/catch 握りつぶしを削除し、`app/api/admin/3d-models/route.ts` GET ハンドラで 500 を返却（運用での障害検知を可能に）。<br>**検証ログ**: `tsc --noEmit` 0 ／ `pnpm lint` clean ／ `pnpm test` 18 files 164 tests passed ／ 本番動作確認 5/5 完了（ai-echo API 形式・履歴ページ・3D 公開非公開トグル・3D 削除・index 適用）。<br>**残課題（Rev39 候補）**: (i) H5: `console.error` 31 箇所を `lib/log.ts:withRequest()` に移行・(ii) Vercel Cron で 3D orphan blob nightly cleanup ジョブ・(iii) R2-O1 Sentry 導入・(iv) R4-O3 admin 内 OpenRouter コストダッシュボード・(v) R4-T3 Playwright E2E。 | CodingAgent / 別 Agent | 2026-05-13 | 約90分 | Rev36 ✅ | ✅完了（本番反映済） |
...
> **CodingAgent 合計推定時間: 約 2385分（39.75時間）** ※Rev07〜23.2＋Rev26＋Rev27＋Rev30＋QA-T1/T2/CI を含む（QA-T3/T4・Rev24/25・Rev28/29 は別枠）
> **完了済み**: Step 01〜22 全て・Rev01〜23.2＋Rev26＋Rev27＋Rev30 全て・Phase QA-T1（スモーク 44/45）／QA-T2（Vitest 46/46）／QA-T3-AiConfig（Vitest 15件追加・合計83 PASS）／QA-CI（基本ジョブ） 🎉
> **未着手（Phase QA）**: QA-T3（残：/api/ai free/pro プラン別）／QA-T4（Playwright E2E・要 chromium install）
> **Phase 4 以降**: Rev24（約4時間・iOS/Android 着手前）／Rev25（記事10,000件規模到達時）
> **Phase 2 以降（A11y）**: Rev28（フォントサイズ調整・約120分・ドラフト）
...
> **Claude コードレビュー対応**: Rev29（Critical 7件・2026-04-25 検証済み・C1-C6 は commits `1d1d7fc`/`14df190` で対応完了・C7 は Rev25 へ合流）／Rev30（AI教室パイプライン本実装＋管理画面＋C1/C2/C4＋S4/S7/S11＋H4＋T1＋H6・2026-04-28 完了・本番デプロイ済み・H2/H3/H5 は junli 判断で不要）／Rev38（rev36/37 自己レビュー High 5件＋本番 DB index 適用・2026-05-13 完了・本番反映済み・H5 と orphan cleanup は Rev39 へ）／Rev39（log 移行 42 件 + 3D orphan cleanup ジョブ実装・本番動作確認で実 7MB 回収成功・2026-05-13 完了・**自動 Cron は誤削除リスク懸念で一時停止中・Rev40 で defensive 設計追加後に復活予定**）
...
> 詳細は `todo/01_システム設計書.md` の「12. モバイルアプリ移行戦略」を参照。
