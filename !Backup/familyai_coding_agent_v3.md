...
```typescript
// middleware.ts または app/api/ai/route.ts に実装すること

// AI API（/api/ai）のレート制限:
// - 未ログインユーザー: IP単位で 10回/日
// - ログイン済みユーザー（無料）: 30回/日
// - ログイン済みユーザー（有料）: 200回/日

// 実装方法（Vercel KV または Upstash Redis を使用）:
import { Ratelimit } from '@upstash/ratelimit';
import { Redis }     from '@upstash/redis';

const ratelimit = new Ratelimit({
  redis: Redis.fromEnv(),
  limiter: Ratelimit.slidingWindow(10, '1 d'), // 未ログイン: 10回/日
});
...
> ⚠️ Upstash Redis は Vercel Marketplace から無料枠で利用可能。
> junliToDo の項目に追加済み。
...
```typescript
// 採用ライブラリ: react-markdown + remark-gfm + rehype-highlight
...
#### ④ CSRF対策
- NextAuth.js v5 はデフォルトで CSRF トークンを使用する
- カスタム API Route（`/api/articles`・`/api/ai` 等）には `next-safe-action` または
  リクエストヘッダーの `Content-Type: application/json` チェックを実装すること
