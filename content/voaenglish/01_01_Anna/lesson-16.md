---
slug: lesson-16
level: beginning
course: anna
courseTitle: "Let's Learn English with Anna"
lessonNumber: 16
title: "What Can Pets Do?"
thumbnail: https://gdb.voanews.com/01000000-0aff-0242-0247-08db0ec58fcc_w400_r1.png
voaUrl: https://learningenglish.voanews.com/a/6983016.html
published: true
# ── R3-機能3（AIctation センテンスプレイヤー骨格）で追加 ──
# ① レッスン概要（日本語）— ① 概要セクションで表示される
description: |
  ペットがテーマ。Anna の猫 Meepers も登場。「**ペットは何ができる？**」（What can pets do?）という質問を通して、所有を表す **my / her**（私の・彼女の）の使い方と、ペットの行動を伝える表現を学びます。
# ② で使う動画 iframe URL（VOA 公式 embed）
videoUrl: https://learningenglish.voanews.com/embed/player/0/6983046.html?type=video
# ② / ③ で使う MP3 ファイルのパス（Vercel Blob 上の相対パス）
# フル URL は env NEXT_PUBLIC_VOA_BLOB_BASE をプレフィックスして合成される
audioPath: voaenglish/01_01_Anna/lesson-16.mp3
---
